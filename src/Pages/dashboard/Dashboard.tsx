...
export function Dashboard() {
  ...
  if (error) {
    return (
      <div className="p-12 text-center bg-white dark:bg-gray-900 rounded-3xl border border-red-100 dark:border-red-900/20 shadow-2xl mx-auto">
  ...
  const tenureColSpan = showTrend ? "lg:col-span-4" : "lg:col-span-12";

  const showReceivables = (revenue?.totalCollected?.paise > 0 || revenue?.totalPending?.paise > 0 || revenue?.totalFailed?.paise > 0);
  const showPlanMix = admins?.byPlan && admins.byPlan.length > 0;
  const showVitality = (reconciliation?.totalDiscrepanciesLast7Days > 0) || (activity?.paymentEventStats && (activity.paymentEventStats.success > 0 || activity.paymentEventStats.failed > 0 || activity.paymentEventStats.ignored > 0));

  const visibleCardsCount = [showReceivables, showPlanMix, showVitality].filter(Boolean).length;
  const gridColsClass = visibleCardsCount === 3
    ? "grid-cols-1 lg:grid-cols-3"
    : visibleCardsCount === 2
      ? "grid-cols-1 lg:grid-cols-2"
      : "grid-cols-1";

  const showInvoices = invoices?.recentInvoices && invoices.recentInvoices.length > 0;
  const showLifecycle = subscriptions?.recentActivity && subscriptions.recentActivity.length > 0;
  const invoicesColSpan = showLifecycle ? "lg:col-span-2" : "lg:col-span-3";
  const lifecycleColSpan = showInvoices ? "lg:col-span-1" : "lg:col-span-3";

  const showPlanPerformance = plans?.adoption && plans.adoption.length > 0;
  const showRecentSignups = admins?.recentSignups && admins.recentSignups.length > 0;
  ...
  return (
    <>
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        {(showTrend || showTenure) && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {showTrend && (
              <ComponentCard
                title="Revenue & Transaction Trends"
                className={`${trendColSpan} overflow-hidden`}
              >
        ...
        {(showReceivables || showPlanMix || showVitality) && (
        ...
        {/* Data Lists Row */}
        {(showInvoices || showLifecycle) && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Recent Invoices */}
            {showInvoices && (
              <div className={invoicesColSpan}>
            ...
            {showLifecycle && (
              <div className={lifecycleColSpan}>
