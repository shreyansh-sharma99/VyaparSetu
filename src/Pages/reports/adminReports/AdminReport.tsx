...
export default function AdminReport() {
  ...
  const showByPlan = byPlanData && byPlanData.length > 0;
  const showStatusBreakdown = data && (data.active > 0 || data.trialing > 0 || data.cancelled > 0 || data.suspended > 0 || data.expired > 0);
  const showRecentAdmins = data.recentAdmins && data.recentAdmins.length > 0;

  const visibleCardsCount = [showByPlan, showStatusBreakdown].filter(Boolean).length;
  ...
  return (
    <>
      ...
      <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        {/* ── Chart Row ─────────────────────────────── */}
        {(showByPlan || showStatusBreakdown) && (
